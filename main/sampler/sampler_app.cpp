...
{
  if (token) {
    for (uint8_t i = 0; i < std::size(builtin_beat_patterns); ++i) {
...
static bool beat_pad_overlap[def::pad::pad_count] = {
...
// Start/End can be marked by ear while the EDIT preview is running. Keep the
// range that launched the voice so changing Start does not move the coordinate
// system used by a subsequent End mark in the same audition.
static bool edit_preview_transport_active = false;
static uint32_t edit_preview_source_start = 0;
...
static bool post_chop_prompt_active = false;
static uint8_t post_chop_count = 0;
static uint8_t post_chop_slot_count = 0;
static uint32_t post_chop_nominal_length_msec = 0;
static bool post_chop_fit_to_beat = false;
static bool post_chop_fn2_engaged = false;
static bool post_chop_fn3_engaged = false;
static constexpr uint32_t post_chop_make_loop_hold_msec = 520;
...
static uint32_t touch_play_motion_msec = 0;
static uint32_t touch_play_motion_sequence = 0;
static uint32_t touch_play_motion_arm_msec = 0;
static bool touch_play_screen_pressed = false;
static bool touch_play_motion_baseline_valid = false;
static bool touch_play_motion_started = false;
static float touch_play_motion_baseline_deg = 0.0f;
static float touch_play_motion_trigger_deg = 0.0f;
static float touch_play_motion_filtered_delta_deg = 0.0f;
...
// 40Hz is responsive for a hand-held tilt while remaining far below the IMU
...
// SAMPLE surface audition is monophonic. Keep it separate from sequenced Pad
// voices so entering Edit or auditioning another Pad never stops the loop.
static int8_t sample_page_audition_pad = -1;
...
// Sequence Mute blocks recorded events, never a newly played Pad. Keep the
// source of the current fixed Pad voice so an old recorded Note Off cannot
// silence a live replacement on the same Pad.
static bool sample_voice_live[def::pad::pad_count] = { false };
static uint16_t sample_sounding_layer[def::pad::pad_count] = { 0 };
// A live Chop gesture owns the immediate beat boundary. Without this short
// guard, the loop clock can dispatch a recorded pre-roll just after the Pad
// task and replace the sound the player intentionally inserted.
static volatile uint32_t chop_live_override_until_msec = 0;
...
static int sample_browser_preview_trigger = -1;
static int8_t shared_sample_assign_pad = -1;
static performance_page_t shared_sample_assign_page = performance_page_t::sample;
enum class sound_pad_release_action_t : uint8_t { none, edit, assign };
static sound_pad_release_action_t sound_pad_release_action = sound_pad_release_action_t::none;
...
static uint32_t beat_voice_age_counter = 1;
...
static bool performance_page_part_muted(performance_page_t page)
{
  ...
  case performance_page_t::bass:
    part_muted = part_muted || mixer_part_muted[(uint8_t)mixer_part_t::bass];
    break;
  case performance_page_t::chord:
    part_muted = part_muted || mixer_part_muted[(uint8_t)mixer_part_t::chord];
    break;
  default:
    break;
  }
...
// SOUND Fn1 uses the menu audition voice so preview never steals a Pad voice
// from the running sequencer. This flag owns only the control-surface state.
static bool sound_page_preview_active = false;
...
static bool performance_status_message_visible(uint32_t now);
static void service_performance_status_overlay(uint32_t now);
...
static void choke_other_sample_pads(int pad, bool allow_chop_overlap = false);
...
static constexpr const mode_info_t mode_info[] = {
  { "SOUND",  0x4080E0u, led_rgb( 32,  96, 255) },
...
static const pad_color_t& pad_colors(int pad) {
  ...
  if (current_mode == sampler_mode_t::mode_rec) {
    const bool valid = current_page == performance_page_t::drum
      ? beat_format == beat_format_t::pattern && beat_pool_t::slot[pad].isValid()
      : sampler_pool_t::slot[pad].isValid();
    if (!valid) { return empty_color; }
...
static sample_slot_t& sample_surface_slot(uint8_t pad)
{
  return current_page == performance_page_t::drum
...
static void update_fn_led(int fn) {
  ...
  if (current_mode == sampler_mode_t::mode_rec && fn == 0
   && sound_page_preview_active && synth_menu_preview_sample_active) {
    active = true;
  }
...
static void invalidate_sample_pad_grid_cache(uint8_t pad)
{
  if (pad >= def::pad::pad_count) { return; }
  // Sampler Pad waveforms are retained in the PLAY-page cache. PCM and trim
  // edits happen on the SAMPLE surface, where request_pad_draw() cannot
  // update that off-screen cache. Clear only the affected tile; the next
  // PLAY presentation rebuilds it without discarding the other eleven Pads.
  const uint8_t index = (uint8_t)performance_page_t::sample;
  grid_cache_pad_mask[index] &= (uint16_t)~(1u << pad);
}

static void invalidate_all_sample_pad_grid_cache(void)
{
  const uint8_t index = (uint8_t)performance_page_t::sample;
...
static size_t compact_sample_assets_now(bool restore_synth_cache)
{
  if (!sampler_pool_t::compactionPending()) { return 0; }
  // Compaction changes the backing PCM address. The caller guarantees that
  // no audio voice is active; clear every prepared cache that still records
  // the former source address before moving the shared ranges. Let the audio
  // task finish its current block after the active flags have gone idle.
  M5.delay(8);
  sampler_audio_t::clearSynthSustainCache();
  for (auto& owner : sampler_sustain_cache_owner) { owner = -1; }
  const size_t reclaimed = sampler_pool_t::compactSparseAssets();
  if (reclaimed && restore_synth_cache) {
...
static void service_sample_asset_compaction(uint32_t now)
{
  if (!sampler_pool_t::compactionPending()
   || loop_playing || recording_pad >= 0
   || performance_record_active || performance_record_finishing
   || processing_screen_visible
   || sound_priority_active() || physical_input_pending()
   || now - sample_asset_compaction_last_attempt_msec < 500) {
    return;
  }
  for (uint8_t voice = 0; voice < sampler_audio_t::max_voice; ++voice) {
    if (sampler_audio_t::isPlaying(voice)) { return; }
  }
  sample_asset_compaction_last_attempt_msec = now;
  compact_sample_assets_now(true);
}
                             ...
                             bool allow_chop_overlap = false)
{
  ...
  if (slot.beatAnchorValid()) {
    edge_fade_in_end = slot.beat_anchor_frame - slot.playStart();
    const uint32_t overlap = std::max<uint32_t>(1, slot.sample_rate * 8u / 1000u);
...
static void choke_other_sample_pads(int pad, bool allow_chop_overlap)
{
  if (pad < 0 || pad >= (int)def::pad::pad_count
  ...
  for (uint8_t other = 0; other < def::pad::pad_count; ++other) {
    ...
    // During transport-synchronised Chop playback the incoming Slice begins
    // in its retained pre-roll. Let the previous Slice reach its own retained
    // tail so the audio-task edge envelopes overlap across the actual beat.
    // Free play still chokes immediately and remains predictable.
    if (allow_chop_overlap && sampler_pool_t::slot[pad].beatAnchorValid()
     && sampler_pool_t::slot[other].beatAnchorValid()) { continue; }
    stop_sample_grid_loop(other);
    if (sample_page_audition_pad == (int8_t)other) { sample_page_audition_pad = -1; }
  }
}

static void arm_live_chop_override(int pad)
{
  if (!loop_playing || pad < 0 || pad >= (int)def::pad::pad_count
   || !sampler_pool_t::slot[pad].beatAnchorValid()) { return; }
  __atomic_store_n(&chop_live_override_until_msec, M5.millis() + 36u,
                   __ATOMIC_RELEASE);
}

static bool recorded_chop_blocked_by_live_input(void)
{
  const uint32_t until = __atomic_load_n(&chop_live_override_until_msec,
                                          __ATOMIC_ACQUIRE);
  return until != 0 && (int32_t)(until - M5.millis()) > 0;
}

static void stop_sample_page_audition(void)
{
  const int pad = sample_page_audition_pad;
  sample_page_audition_pad = -1;
  // During loop playback the same fixed Pad voices belong to the sequencer.
  // SAMPLE does not audition Pads in that state, so never stop a loop voice
  // merely because an older audition marker remained set.
  if (pad < 0 || loop_playing) { return; }
  stop_sample_grid_loop(pad);
  request_pad_state_draw(pad);
}

static void play_sample_page_audition(int pad)
{
  stop_sample_page_audition();
  play_sample_once(pad);
  sample_page_audition_pad = (int8_t)pad;
}
...
static void service_wave_transfer(void)
{
  ...
  if (performance_status_message_visible(M5.millis())) { return; }
...
static void service_sample_preview_cursor(uint32_t now)
{
  ...
  if (performance_status_message_visible(now)) {
...
static void service_loop_length_label_overlay(void)
{
  if ((!loop_length_label_overlay_pending && !loop_length_label_restore_pending)
   ...
   || performance_status_message_visible(M5.millis())) {
    return;
  }
...
static void draw_sample_move_panel(M5Canvas& c)
{
  ...
  if (copy_ready) {
    ...
    c.drawString("RELEASE SOURCE: EDIT", w / 2, h - 48);
...
static bool fn_information_chip_bounds(int* x, int* y, int* w, int* h)
{
  ...
  int chip_w = sample_edit_armed
              && current_page != performance_page_t::sample ? 224
              : sample_edit_armed || delete_confirm ? 184
...
static bool draw_fn_information_chip(M5Canvas& c)
{
  ...
  if (play_loop_grid_information_active()) {
    ...
    const bool synth_sound = current_page == performance_page_t::melody
                          || current_page == performance_page_t::bass
                          || current_page == performance_page_t::chord;
    const bool assigned = synth_sound
                       && page_settings(current_page).source == synth_tone_source_t::pad
                       && page_settings(current_page).pad == (uint8_t)sample_edit_armed_pad;
    c.setTextColor(0xD8E8FFu);
    c.drawString(synth_sound && !assigned ? "TAP AGAIN: SET" : "TAP AGAIN: EDIT",
                 chip_x + chip_w / 2, chip_y + 13);
    c.setTextColor(0xFFFFFFu);
    c.drawString(synth_sound && !assigned ? "HOLD: EDIT / MOVE" : "HOLD: MOVE",
                 chip_x + chip_w / 2, chip_y + 31);
...
static void draw_wave(void) {
  ...
  if (current_page != performance_page_t::sample
   ...
   && !(current_mode == sampler_mode_t::mode_loop
    ...
    if (current_page == performance_page_t::melody) {
      ...
      title = "BEAT";
      snprintf(lines[line_count++], sizeof(lines[0]), "Type   : %s",
        beat_format == beat_format_t::audio ? "Audio"
        : beat_format == beat_format_t::pattern ? "Pattern" : "Empty");
      if (beat_name[0]) {
        snprintf(lines[line_count++], sizeof(lines[0]), "Sound  : %.32s", beat_name);
      }
      snprintf(lines[line_count++], sizeof(lines[0]), "Volume : %u%%", (unsigned)beat_volume);
    }
  ...
  if (current_mode == sampler_mode_t::mode_rec) {
    if (current_page == performance_page_t::drum
     && beat_format == beat_format_t::audio && background_loop.isValid()) {
      for (int x = 0; x < w; ++x) {
        const uint32_t a = ((uint64_t)x * background_loop.frames) / w;
        uint32_t b = ((uint64_t)(x + 1) * background_loop.frames) / w;
        if (b <= a) { b = a + 1; }
        b = std::min<uint32_t>(b, background_loop.frames);
        const uint32_t stride = std::max<uint32_t>(1, (b - a) / 48u);
        int16_t mn = INT16_MAX;
        int16_t mx = INT16_MIN;
        for (uint32_t frame = a; frame < b; frame += stride) {
          mn = std::min(mn, background_loop.pcm[frame]);
          mx = std::max(mx, background_loop.pcm[frame]);
        }
        const int y0 = (h / 2) - ((int)mx * (h / 2 - 5)) / 32768;
        int y1 = (h / 2) - ((int)mn * (h / 2 - 5)) / 32768;
        if (y1 <= y0) { y1 = y0 + 1; }
        c.drawFastVLine(x, y0, y1 - y0, performance_page_colors[(uint8_t)current_page]);
      }
      c.setFont(&fonts::efontJA_16_b);
      c.setTextSize(1);
      c.setTextDatum(m5gfx::textdatum_t::top_left);
...
static void draw_pad_content(m5gfx::LovyanGFX& d, int pad, int origin_x = 0, int origin_y = 0)
{
  ...
  if (current_page == performance_page_t::drum && beat_format != beat_format_t::pattern) {
    background = pad_off_background(empty_color);
  }
    ...
    if (current_page == performance_page_t::melody
     || current_page == performance_page_t::bass) {
      ...
      snprintf(label, sizeof(label), "%s",
               beat_format == beat_format_t::pattern ? beat_pad_labels[order] : "");
    }
...
static void draw_fn_content(m5gfx::LovyanGFX& d, int fn, int origin_x = 0, int origin_y = 0)
{
  ...
  if (post_chop_prompt_active || post_chop_fn2_engaged || post_chop_fn3_engaged) {
    if (fn == 0) {
      ...
      d.drawString(fn == 1 ? "OK" : "BACK", cx, cy);
    }
    return;
  }
  ...
  case sampler_mode_t::mode_rec: {
    if (fn == 0) {
      const bool playing = sound_page_preview_active && synth_menu_preview_sample_active;
      draw_icon(d, playing ? icon_t::stop : icon_t::volume, cx, cy, s,
                active ? 0xFFFFFFu : playing ? 0xFF7070u : edit_preview_label_color, bg);
    } else if (fn == 1) {
      draw_icon(d, icon_t::mute, cx, cy, s, color, bg);
    } else if (current_page == performance_page_t::sample) {
  ...
  case sampler_mode_t::mode_play: {
    if (fn == 0) {
      ...
      draw_icon(d, icon_t::mute, cx, cy, s, color, bg);
    } else if (current_page == performance_page_t::melody || current_page == performance_page_t::bass) {
      d.setFont(&fonts::efontJA_16_b);
      d.setTextSize(1);
      d.setTextDatum(m5gfx::textdatum_t::middle_center);
...
static void flush_dirty_ui(bool force = false)
{
  ...
  if (dirty_wave && !wave_transfer_active
   && !performance_status_message_visible(now)) {
    dirty_wave = false;
    draw_wave();
    if (!force) { return; }
  }
...
static bool performance_status_overlay_drawn = false;
...
{
  const uint8_t value = (uint8_t)std::min<uint32_t>(127, ((uint32_t)beat_volume * 127 + 50) / 100);
...
static void draw_wifi_compact_status(wifi_radio_request_t request)
{
  ...
  d.drawString(request == wifi_radio_request_t::update_check
    ? "Press any key to cancel" : "Wi-Fi is starting",
    d.width() / 2, d.height() / 2 + 22);
  d.endWrite();
}
...
static void menu_value_set(menu_value_t value, int index)
{
  ...
  case menu_value_t::background_repeat: {
    ...
    set_beat_repeat(repeats[std::min<int>(index, 2)]);
    break; }
  case menu_value_t::midi_input: {
    using namespace kp::def::command;
...
static bool performance_status_message_visible(uint32_t now)
{
  return !menu_visible && learn_state == learn_state_t::idle
      && !ui_surface_exclusive && !performance_ui_arena_suspended
      && status_message_visible(now);
}

static const char* post_chop_prompt_action_text(void)
{
  return "HOLD: MAKE LOOP";
}
...
static void service_performance_status_overlay(uint32_t now)
{
  ...
  if (hold_progress_kind != hold_progress_kind_t::none || page_selector_visible) {
    // These transient controls own the same region. Redraw the notice after
    // they close instead of assuming its previous pixels survived.
    performance_status_overlay_drawn = false;
    return;
  }
  if (performance_status_overlay_drawn || ui_async_display_busy()
   || ui_surface_exclusive
   || performance_ui_arena_suspended) { return; }

  // Performance messages are a direct, compact top layer. The retained Wave
  // Canvas remains untouched underneath and is restored once at teardown.
  const bool sample_add_prompt = sample_add_armed_pad >= 0;
  const bool post_chop_prompt = post_chop_prompt_active;
  const int chip_w = 224;
  const int chip_h = post_chop_prompt ? 70 : sample_add_prompt ? 52 : 34;
  const int chip_x = (M5.Display.width() - chip_w) / 2;
  const int chip_y = wave_y + (wave_h - chip_h) / 2;
  const uint32_t bg = 0x202030u;
  const uint32_t accent = performance_page_colors[(uint8_t)current_page];
  auto& d = M5.Display;
  d.startWrite();
  d.fillRoundRect(chip_x, chip_y, chip_w, chip_h, 5, bg);
  d.drawRoundRect(chip_x, chip_y, chip_w, chip_h, 5, accent);
  d.setFont(&fonts::efontJA_16_b);
  d.setTextSize(1);
  d.setTextDatum(m5gfx::textdatum_t::middle_center);
  if (sample_add_prompt) {
    // This is a choice, not an error: keep Load neutral and reserve red for
    // the destructive/capturing action so the long-press meaning is obvious.
    d.setTextColor(0xD8E8FFu, bg);
    d.drawString("TAP: IMPORT", M5.Display.width() / 2, chip_y + 15);
    d.setTextColor(0xFF6868u, bg);
    d.drawString("HOLD: RECORD", M5.Display.width() / 2, chip_y + 37);
  } else if (post_chop_prompt) {
    d.setTextColor(0xF0C050u, bg);
    d.drawString("CHOP COMPLETE", M5.Display.width() / 2, chip_y + 14);
    d.setTextColor(0xD8E8FFu, bg);
    d.drawString("TAP: DONE", M5.Display.width() / 2, chip_y + 35);
    d.setTextColor(0xFFFFFFu, bg);
    d.drawString(post_chop_prompt_action_text(), M5.Display.width() / 2, chip_y + 56);
  } else {
    d.setTextColor(0xFFFFFFu, bg);
    d.drawString(status_message, M5.Display.width() / 2, chip_y + chip_h / 2);
  }
  d.endWrite();
  performance_status_overlay_drawn = true;
}
...
static void show_status_message(const char* msg, uint32_t duration_ms = 1600, bool redraw = true)
{
  ...
  performance_status_overlay_drawn = false;
  if (!menu_visible) {
    // Cancel a queued Wave strip before it can arrive on Core 0 after the
    // popup and repaint over it. The underlying canvas itself is preserved.
    wave_transfer_generation = wave_transfer_generation + 1;
    wave_transfer_active = false;
    wave_transfer_full_frame = false;
  }
...
static void update_menu_keypad_leds()
{
  ...
  for (int btn = 0; btn < 15; ++btn) {
    ...
    const bool command = confirm_import ? (btn == 4 || btn == 9)
...
static void draw_menu_keypad(bool force)
{
  ...
  if (kit_edit_state == kit_edit_state_t::assign_wait_pad
   || kit_edit_state == kit_edit_state_t::assign_confirm_shortcut
   || kit_edit_state == kit_edit_state_t::clear_wait_pad) {
    const bool confirm_import = kit_edit_state == kit_edit_state_t::assign_confirm_shortcut;
    for (int pad = 0; pad < (int)def::pad::pad_count; ++pad) {
      draw_pad(pad);
    ...
    for (int fn = 0; fn < 3; ++fn) {
      ...
      const bool back = btn == 4;
      const bool ok = confirm_import && btn == 9;
      uint32_t bg = back ? 0x483030u : ok ? 0x304838u : 0x202028u;
      uint32_t fg = back ? menu_back_label_color : ok ? menu_ok_label_color : 0x606078u;
      ...
      if (back || ok) {
        ...
        d.setTextDatum(m5gfx::textdatum_t::middle_center);
        d.setTextColor(fg, bg);
        d.drawString(back ? "Back" : "OK", fn_x + fn_w / 2, y + cell_h / 2);
      }
    }
    const int focused_pad = confirm_import ? kit_shortcut_target_pad : kit_assign_loading_pad;
    if ((confirm_import || kit_edit_state == kit_edit_state_t::assign_wait_pad)
     && focused_pad >= 0 && focused_pad < (int)def::pad::pad_count) {
      const int x = grid_x + (focused_pad % 4) * col_pitch;
      const int y = grid_y + (focused_pad / 4) * row_pitch;
  ...
  for (int btn = 0; btn < 15; ++btn) {
    ...
    const bool adjust = value_adjust && (btn == 13 || btn == 14);
    const bool preview = file_preview && btn == 14;
    bool command = confirm_import ? (btn == 4 || btn == 9)
                              : wait_pad ? (btn == 4) : (btn == 4 || btn == 8 || btn == 9
                                         || adjust || preview || (input_assignment_list_active && btn == 14));
    uint32_t bg = command ? 0x263048u : 0x202028u;
...
static void draw_wifi_update_overlay(void)
{
  ...
  if (kp::system_registry->wifi_control.getOperation() == kp::def::command::wifi_operation_t::wfop_ota_begin) {
    ...
    d.drawString("Press any key: Cancel", d.width() / 2, 213);
  }
  d.endWrite();
}
...
static void cancel_startup_update_check(void)
{
  ...
  auto reg = kp::system_registry;
  using kp::def::command::wifi_operation_t;
  // Cancellation is safe while BLE is being released or while STA is still
  // connecting. Once the catalog HTTP request starts, keep input blocked and
  // let the short transaction finish without tearing down its network state.
  if (wifi_radio_request != wifi_radio_request_t::update_check
   && reg->wifi_control.getOperation() != wifi_operation_t::wfop_update_check_begin) { return; }

  startup_update_check_pending = false;
  startup_update_check_active = false;
  startup_update_check_returning = true;
  disable_wifi_and_clear_indicator();
  ...
  d.setTextDatum(m5gfx::textdatum_t::middle_center);
  d.setTextColor(0xFFFFFFu, 0x08080Cu);
  d.drawString("CANCELLED", d.width() / 2, d.height() / 2);
  d.endWrite();
  // disable_wifi_and_clear_indicator() restores the retained performance UI
  // after the Wi-Fi task has released its PSRAM/network allocations.
}
...
static bool render_menu_content(M5Canvas& d, int scroll_px = 0)
{
  ...
  if (kit_edit_state == kit_edit_state_t::assign_confirm_shortcut) {
    ...
    snprintf(target, sizeof(target), "Assign to P%u",
             kit_shortcut_target_pad >= 0
               ? (unsigned)pad_display_number((uint8_t)kit_shortcut_target_pad) : 0u);
    // The header already says "Import to Pxx". Keep the content focused on
    // the destination and source name instead of repeating menu navigation.
    draw_menu_wait_pad(d, nullptr, target, kit_pending_wav_name);
    return true;
  }
...
static bool is_beat_file_name(const std::string& name)
{
  return is_audio_file_name(name)
      || has_lower_suffix(name, ".mid") || has_lower_suffix(name, ".midi");
}
...
static bool load_menu_beat_file_list_from(const char* dir)
{
  kit_wav_list.clear();
  if (!dir || !dir[0]) { return false; }
  kp::storage_sd.getFileList(kit_wav_list, dir, "");
  kit_wav_list.erase(std::remove_if(kit_wav_list.begin(), kit_wav_list.end(),
    [](const kp::file_info_string_t& f) { return !is_beat_file_name(f.filename); }), kit_wav_list.end());
  std::sort(kit_wav_list.begin(), kit_wav_list.end(),
    [](const kp::file_info_string_t& a, const kp::file_info_string_t& b) { return a.filename < b.filename; });
  if (kit_wav_list.empty()) { return false; }
  snprintf(kit_wav_dir, sizeof(kit_wav_dir), "%s", dir);
...
static bool synth_sound_surface_active(void)
{
  return current_mode == sampler_mode_t::mode_rec
      && (current_page == performance_page_t::bass
       || current_page == performance_page_t::melody
...
static void arm_synth_sound_pad(uint8_t pad)
{
  ...
  shared_sample_assign_page = current_page;
...
static void assign_pending_wav_to_pad(uint8_t pad)
{
  ...
  if (shortcut_import && ok) {
    const bool arm_for_synth = synth_sound_surface_active();
    kit_shortcut_target_pad = -1;
    kit_assign_loading_pad = -1;
    kit_edit_state = kit_edit_state_t::idle;
    menu_close();
    if (arm_for_synth) {
      arm_synth_sound_pad(pad);
      char ready[32];
      snprintf(ready, sizeof(ready), "SAMPLE READY: P%u",
               (unsigned)pad_display_number(pad));
      show_status_message(ready, 1200, false);
    } else {
...
{
  if (wifi_update_active || startup_update_check_active || startup_update_check_returning
   || wifi_file_server_qr_active || controller >= 128) { return; }
  if (learn_state == learn_state_t::waiting_external) {
...
{
  const bool input_locked = wifi_update_active || startup_update_check_active
  ...
  while ((h = input.getHistory(external_input_history_code)) != nullptr) {
    ...
    // Consume transitions while Wi-Fi owns the foreground. Replaying stale
    // button edges after the check ends would be worse than ignoring them.
    if (input_locked) { continue; }
    while (pressed_edge) {
...
static void process_usb_keyboard_input(void)
{
  ...
  while (task_midi.getUSBHIDKeyboardEvent(&key, &pressed)) {
    if (!usb_keyboard_enabled || wifi_update_active || startup_update_check_active
     || startup_update_check_returning
...
static void process_usb_gamepad_input(void)
{
  ...
  while (task_midi.getUSBHIDGamepadEvent(&code, &pressed)) {
    if (!usb_gamepad_enabled || wifi_update_active || startup_update_check_active
     || startup_update_check_returning
     || wifi_file_server_qr_active) { continue; }
    if (learn_state == learn_state_t::waiting_external) {
...
static void loop_remove_page_pad_events(performance_page_t page, int pad)
{
  ...
  if (loop_events.empty()) {
    if (background_loop.isValid()) {
      ...
      // With no recorded part and no Audio Beat there is no musical cycle to
      // preserve. Return to fresh-capture state immediately, including when
      // the final event was removed while the transport was running.
...
static void loop_reset_recording_state_if_empty(void)
{
  ...
  // A completely empty pattern has no length. This single reset path is used
  // by per-Pad delete, part clear and menu Clear Pattern, so the next hit
  // always starts a new capture consistently.
  loop_reset_recording_state();
}
...
#if !defined (M5UNIFIED_PC_BUILD)
static bool prepare_internal_mic_recording(void)
{
  // M5Unified starts the CoreS3 microphone at its default 16 kHz during boot.
  // Merely checking isRunning() can therefore leave the first capture clocked
  // at that old rate while the Pad metadata says 32 kHz.  Rebuild the input
  // path at the requested rate before every internal-mic take.  The zero-size
  // request also commits Mic_Class' private rate cache without consuming PCM.
  while (M5.Mic.isRecording()) { M5.delay(1); }
  if (M5.Mic.isRunning()) { M5.Mic.end(); }
  M5.Mic.setSampleRate(recording_internal_sample_rate);
  if (!M5.Mic.begin()) { return false; }
  if (!M5.Mic.record(recording_buffer, 0, recording_internal_sample_rate)) {
    M5.Mic.end();
    return false;
  }
  return true;
...
static void start_pad_recording(int pad)
{
  ...
  cancel_sample_move();
  bool stopped_for_compaction = false;
  if (sampler_pool_t::compactionPending()) {
    clear_synth_runtime();
    sampler_audio_t::stopAll();
    clear_sample_grid_loops();
  ...
  if (recording_source_mode == recording_source_mode_t::external_input) {
    ...
    if (looks_like_external_input(recording_buffer, external_frames)) {
      ...
      if (!prepare_internal_mic_recording()) {
        sampler_audio_t::setOutputMuted(false);
        release_recording_buffer_if_idle();
        schedule_internal_synth_restore();
        return;
...
static void finish_pad_recording(void)
{
  ...
  if (auto_crop_recording(recording_buffer, frames, sample_rate, crop)) {
    ...
    loop_reset_recording_state_if_empty();
    // The capture workspace used to be copied into another full-size Pad
    // allocation, briefly requiring two Long Samples. Shrink it to the take
    // length and transfer ownership directly to the Sample pool instead.
    const size_t recorded_bytes = (size_t)frames * sizeof(int16_t);
#if defined (M5UNIFIED_PC_BUILD)
    int16_t* resized_recording = (int16_t*)realloc(recording_buffer, recorded_bytes);
#else
    int16_t* resized_recording = (int16_t*)heap_caps_realloc(
      recording_buffer, recorded_bytes, MALLOC_CAP_SPIRAM);
#endif
    if (resized_recording) { recording_buffer = resized_recording; }
    bool loaded_recording = resized_recording
      && sampler_pool_t::loadRecordedPcmOwned(
        (uint8_t)pad, name, recording_buffer, frames, sample_rate);
    if (loaded_recording) {
      recording_buffer = nullptr;  // Sample Asset now owns the allocation.
    } else if (recording_buffer) {
      // A rare realloc/adoption failure keeps the previous safe copy path.
      loaded_recording = sampler_pool_t::loadRecordedPcm(
        (uint8_t)pad, name, recording_buffer, frames, sample_rate);
    }
    if (loaded_recording) {
      auto& slot = sampler_pool_t::slot[pad];
      ...
      if (synth_sound_surface_active()) {
        arm_synth_sound_pad((uint8_t)pad);
        char ready[32];
        snprintf(ready, sizeof(ready), "SAMPLE READY: P%u",
                 (unsigned)pad_display_number((uint8_t)pad));
        show_status_message(ready, 1200, false);
      } else {
...
static void preview_edit_transport(bool press)
{
  ...
  if (press) {
    edit_preview_source_start = slot.playStart();
    edit_preview_source_end = slot.playEnd();
    edit_preview_source_reverse = slot.reverse;
    edit_preview_transport_active = true;
...
static bool mark_edit_trim_from_preview(uint8_t parameter)
{
  if ((parameter != 0 && parameter != 1) || !edit_preview_transport_active
   || edit_pad < 0 || edit_pad >= (int)def::pad::pad_count
   || edit_synth_page || edit_chop_page) {
    return false;
  }
  ...
  auto& slot = sampler_pool_t::slot[edit_pad];
  if (!slot.isValid() || edit_preview_source_end <= edit_preview_source_start) {
    return false;
  }
  const uint32_t source_span = edit_preview_source_end - edit_preview_source_start;
  const uint32_t local = std::min<uint32_t>(frame, source_span - 1);
  uint32_t source_frame = edit_preview_source_reverse
    ? edit_preview_source_end - 1 - local
    : edit_preview_source_start + local;

  // Preserve the player's timing while avoiding a new hard edge in the PCM.
  // A 2ms search is short enough that the marked point still feels immediate.
  const uint32_t zero_radius = std::max<uint32_t>(1, slot.sample_rate / 500);
  source_frame = find_nearest_zero_crossing(slot.pcm, slot.frames,
                                             source_frame, zero_radius);
  const uint32_t minimum = std::min<uint32_t>(256, slot.frames);
  if (parameter == 0) {
    const uint32_t end = slot.playEnd();
    slot.start_frame = std::min<uint32_t>(source_frame,
      end > minimum ? end - minimum : 0);
  } else {
    const uint32_t start = slot.playStart();
    slot.end_frame = std::clamp<uint32_t>(source_frame,
      std::min<uint32_t>(slot.frames, start + minimum), slot.frames);
  }
...
static void enter_edit(int pad)
{
  ...
  // The second tap changes the Pad from audition to editing. Silence the
  // audition before the editor cursor starts using the same voice.
  stop_sample_page_audition();
...
static uint8_t chop_loop_slot_count(uint8_t chop_count)
{
  return chop_count <= 6 ? 4 : 8;
}

static bool length_at_least_150_percent(uint32_t length, uint32_t reference)
{
  return (uint64_t)length * 100u >= (uint64_t)reference * 150u;
}

static bool length_below_75_percent(uint32_t length, uint32_t reference)
{
  return (uint64_t)length * 100u < (uint64_t)reference * 75u;
}

// Move a musically equivalent duration by octaves until only a modest tempo
// correction remains. This is shared by FIT and the post-Chop Rec transform,
// so choosing DONE or MAKE LOOP cannot reinterpret the same phrase length.
static uint32_t octave_match_length(uint32_t candidate, uint32_t reference,
                                    uint32_t minimum, uint32_t maximum)
{
  candidate = std::clamp<uint32_t>(candidate, minimum, maximum);
  while (length_at_least_150_percent(reference, candidate)
      && candidate <= maximum / 2u) {
    candidate *= 2u;
  }
  while (length_below_75_percent(reference, candidate) && candidate > minimum) {
    const uint32_t next = std::max<uint32_t>(minimum, candidate / 2u);
...
static uint32_t chop_fit_frames(uint32_t source_frames, uint32_t sample_rate,
                                uint32_t reference_ms, uint8_t chop_count)
{
  if (!reference_ms || !sample_rate || !chop_count) { return source_frames; }
  const uint64_t reference_frames = ((uint64_t)reference_ms * sample_rate + 500) / 1000;
  const uint64_t max_frames = (uint64_t)sample_rate * sampler_pool_t::max_sample_sec;
  const uint8_t slots = chop_loop_slot_count(chop_count);

  // The musical unit is not the complete source when Chop count is 5, 6 or
  // 9-12. It is the duration occupied by the first 4/8 equal slices.
  const uint32_t natural_phrase = (uint32_t)std::max<uint64_t>(1,
    ((uint64_t)source_frames * slots + chop_count / 2u) / chop_count);
  uint32_t minimum_phrase = 1;
  uint32_t maximum_phrase = (uint32_t)std::min<uint64_t>(UINT32_MAX, max_frames);
  if (background_loop.isValid() && beat_format == beat_format_t::audio
   && background_loop.sample_rate) {
    // Audio Beat can only change transport length by repeating its actual PCM
    // cycle. Keep FIT inside the public 1/2/4 Repeat choices.
    const uint64_t beat_cycle = ((uint64_t)background_loop.frames * sample_rate
                               + background_loop.sample_rate / 2u)
                              / background_loop.sample_rate;
    minimum_phrase = (uint32_t)std::max<uint64_t>(1, beat_cycle);
    maximum_phrase = (uint32_t)std::min<uint64_t>(max_frames, beat_cycle * 4u);
  }
  const uint32_t fitted_phrase = octave_match_length(
    (uint32_t)std::clamp<uint64_t>(reference_frames, minimum_phrase, maximum_phrase),
    natural_phrase, minimum_phrase, maximum_phrase);
  return (uint32_t)std::clamp<uint64_t>(
    ((uint64_t)fitted_phrase * chop_count + slots / 2u) / slots,
    1, max_frames);
}
                                      ...
                                      uint8_t chop_count)
{
  ...
  const uint32_t target_frames = chop_fit_frames(
    source_frames, sample_rate, reference_length, chop_count);
...
static bool finish_tap_cut_capture(void)
{
  ...
  if (!build_chop_slice_plan_from_boundaries(
   ...
   || !update_chop_preview_pitch(edit_chop_tap_source_frames, source.sample_rate, count)) {
    reset_chop_preview();
    return false;
  }
...
static bool prepare_chop_preview_plan(void)
{
  ...
  if (!update_chop_preview_pitch(source_frames, source.sample_rate, count)) { return false; }
                                               ...
                                               float margin_per_window = 0.08f)
{
  ...
  // Long source PCM and Chop assets live in PSRAM. Keep this temporary
  // analysis window there as well: allocating it from the small internal heap
  // can fail after recording/editing even when plenty of PSRAM remains, which
  // previously looked like an unpitched source to the user.
  float* windowed = (float*)temp_alloc(sizeof(float) * analysis_points);
...
static bool chop_edit_sample(void)
{
  ...
  const uint32_t working_frames = fit_to_bgm
    ? chop_fit_frames(source_frames, source.sample_rate, reference_length, chop_count)
    : source_frames;
  draw_recording_processing_frame("FINDING KEY");
  ...
  // CHOP itself ends after assigning Pads. Retain only its detected pulse and
  ...
  post_chop_fn3_engaged = false;
  save_resume_kit();
  processing_screen_visible = false;
  // Reapply both SAM2695 programs and Pad-synth caches only after the long
  // synchronous Chop/save path has returned ownership to the normal loop.
  // Sending this earlier can leave the MIDI queue or a replaced Pad cache in
  // the pre-Chop state, which presents as all synth parts becoming silent.
  sampler_audio_t::setOutputMuted(false);
  schedule_internal_synth_restore(100);
  if (current_page != performance_page_t::sample) {
    set_performance_page(performance_page_t::sample);
  }
  if (current_mode != sampler_mode_t::mode_rec) { set_mode(sampler_mode_t::mode_rec); }
  set_rec_wave_pad(targets[0]);
  post_chop_prompt_active = true;
  show_status_message("CHOP COMPLETE", 0, false);
  draw_all();
  update_all_leds();
  return true;
}
...
static void handle_edit_function_pad(int pad)
{
  ...
  case 8:
    ...
    break;
  case 9:
    if (mark_edit_trim_from_preview(0)) { return; }
    edit_param = 0;
    break;
  case 10:
    if (mark_edit_trim_from_preview(1)) { return; }
    edit_param = 1;
    break;
...
static bool performance_pad_has_sound(performance_page_t page, uint8_t pad)
{
  ...
  if (page == performance_page_t::drum) {
    return beat_format == beat_format_t::pattern
        && pad < def::pad::pad_count
        && beat_pool_t::slot[pad].isValid();
  }
  uint8_t notes[4];
...
{
  if (pad >= def::pad::pad_count) { return; }
  invalidate_sample_pad_grid_cache(pad);
...
static void stop_beat_voices(bool include_live)
{
  for (uint8_t voice = 0; voice < beat_voice_count; ++voice) {
    if (!include_live && beat_voice_live[voice]) { continue; }
    sampler_audio_t::stop(beat_voice_base + voice);
    beat_voice_pad[voice] = -1;
    beat_voice_live[voice] = false;
    beat_voice_age[voice] = 0;
  }
}

static void stop_beat_sample_page_audition(void)
{
  // Recorded Beat voices use the same pool. Stop only live audition voices,
  // preserving the pattern currently running underneath the SAMPLE surface.
  for (uint8_t voice = 0; voice < beat_voice_count; ++voice) {
    if (!beat_voice_live[voice]) { continue; }
    sampler_audio_t::stop(beat_voice_base + voice);
    beat_voice_pad[voice] = -1;
    beat_voice_live[voice] = false;
    beat_voice_age[voice] = 0;
...
static void trigger_beat_pad(uint8_t pad, bool live_performance)
{
  ...
  // Closed/Open hats share the single beginner-facing Beat choke group.
  // Other sounds overlap naturally. The group is stored per Pad so a future
  // Beat editor can expose it without changing the playback engine.
  if (!beat_pad_overlap[pad]) {
    for (uint8_t voice = 0; voice < beat_voice_count; ++voice) {
      const int8_t owner = beat_voice_pad[voice];
      if (owner >= 0 && owner < (int8_t)def::pad::pad_count
       && !beat_pad_overlap[(uint8_t)owner]) {
        sampler_audio_t::stop(beat_voice_base + voice);
        beat_voice_pad[voice] = -1;
        beat_voice_live[voice] = false;
        beat_voice_age[voice] = 0;
  ...
  uint32_t oldest = UINT32_MAX;
  for (uint8_t voice = 0; voice < beat_voice_count; ++voice) {
    if (!sampler_audio_t::isPlaying(beat_voice_base + voice)) {
      selected = voice;
      oldest = 0;
      break;
    }
    if (beat_voice_age[voice] < oldest) {
      oldest = beat_voice_age[voice];
      selected = voice;
    }
  }
  ...
  beat_voice_pad[selected] = (int8_t)pad;
  beat_voice_live[selected] = live_performance;
  beat_voice_age[selected] = beat_voice_age_counter++;
  if (beat_voice_age_counter == 0) { beat_voice_age_counter = 1; }
}
                              ...
                              bool live_performance)
{
  ...
  auto& state = synth_trigger_state[(uint8_t)page][pad];
  if (live_performance) {
    // A direct live replacement has no recorded layer. This prevents a
    // recorded Note Off crossing the transport cursor from stopping it.
    synth_sounding_layer[(uint8_t)page][pad] = 0;
  }
  state.live = live_performance;
  state.note_count = note_count;
  const uint8_t volume = page_settings(page).volume;
  if (page_settings(page).source == synth_tone_source_t::general_midi) {
    state.midi = true;
    uint8_t channel = page_midi_channel(page);
...
static float touch_play_wrap_angle(float degrees)
{
...
static bool touch_play_read_motion_pitch(float* degrees, uint32_t* sequence = nullptr)
{
  ...
  if (!kp::system_registry
   || !kp::system_registry->internal_imu.getAccel(
        &accel_x, &accel_y, &accel_z, &sample_sequence)) {
    return false;
  }
  (void)accel_y;
  // KANTAN Play mounts CoreS3 sideways: body Up is -X, body Right is +Y.
  // Rotation around body Right (+Y) is therefore the up/down Kaoss gesture.
  // Ignore the singular pose where gravity lies almost completely on body Y.
  const int64_t pitch_plane_energy = (int64_t)accel_x * accel_x
                                   + (int64_t)accel_z * accel_z;
  if (pitch_plane_energy < 2048ll * 2048ll) { return false; }
  if (degrees) {
    *degrees = -atan2f((float)accel_x, (float)accel_z)
             * (180.0f / 3.14159265358979323846f);
  }
  if (sequence) { *sequence = sample_sequence; }
  return true;
}

static void touch_play_rearm_motion(uint32_t now, bool keep_baseline)
{
  float angle = 0.0f;
  uint32_t sequence = 0;
  const bool valid = touch_play_read_motion_pitch(&angle, &sequence);
  if (valid) {
    touch_play_motion_sequence = sequence;
    touch_play_motion_trigger_deg = angle;
    if (!keep_baseline || !touch_play_motion_baseline_valid) {
...
static int touch_play_motion_pad(float angle)
{
  ...
  if (!touch_play_motion_started) {
    touch_play_motion_filtered_delta_deg = delta;
  } else {
    // A small low-pass removes hand tremor without delaying the first attack.
    touch_play_motion_filtered_delta_deg +=
      (delta - touch_play_motion_filtered_delta_deg) * 0.35f;
  }
  const float position = (touch_play_motion_filtered_delta_deg + 90.0f) / 180.0f;
  const uint8_t order = (uint8_t)std::clamp<int>(
    (int)lroundf(position * (def::pad::pad_count - 1)),
    0, def::pad::pad_count - 1);
  return display_order_to_pad(order);
}
...
{
  if (!touch_play_active) { return; }
  touch_play_screen_pressed = pressed;
  if (!pressed) {
    if (touch_play_pad >= 0) { release_synth_trigger(current_page, (uint8_t)touch_play_pad); }
    touch_play_pad = -1;
    touch_play_target_pad = -1;
    // Returning from direct screen control must stay silent until the user
    // makes another deliberate movement. Keep the original Fn3 baseline so
    // the full +/-90-degree pitch range does not jump.
    touch_play_rearm_motion(M5.millis(), true);
    return;
  }
...
static void set_touch_play_active(bool active)
{
  if (active) {
    ...
    // Silence only sustained notes started by the recorded sequence. A note
    // the player is physically holding remains live as Touch Play opens.
    for (uint8_t pad = 0; pad < def::pad::pad_count; ++pad) {
      auto& state = synth_trigger_state[(uint8_t)current_page][pad];
      if (state.note_count != 0 && !state.live) {
        release_synth_trigger(current_page, pad);
        synth_sounding_layer[(uint8_t)current_page][pad] = 0;
      }
    }
    touch_play_pad = -1;
    touch_play_target_pad = -1;
    ...
    touch_play_motion_sequence = 0;
    touch_play_rearm_motion(now, false);
...
static void loop_finish_length_capture(uint32_t now)
{
  ...
  if (beat_format == beat_format_t::pattern) {
    // A repeat chosen before the first recording treats the captured gesture
    // as one Pattern cycle, then expands it. This keeps the product BPM-free
                               ...
                               bool start_at_anchor = false)
{
  if (loop_event_is_pitch_bend(event.type)) {
    ...
    if (!live_input && performance_page_part_muted(event.page)) { return; }
  ...
  if (!slot.isValid() || slot.playFrames() == 0) { return; }
  if (event.type == loop_event_type_t::note_off) {
    // Recorded releases belong only to the exact recorded layer. A live Pad
    // pressed while its sequence is muted must remain under the player's
    // finger even when an old loop Note Off passes the transport cursor.
    if (event.layer != 0 && sample_sounding_layer[pad] != event.layer) { return; }
  ...
  if (!live_input && slot.beatAnchorValid()
   && recorded_chop_blocked_by_live_input()) {
    return;
  }
  if (live_input) { arm_live_chop_override(pad); }
  if (event.layer != 0 && loop_deferred_note_on_layer[pad] == event.layer) {
    loop_deferred_note_on_layer[pad] = 0;
  }
  if (!live_input && loop_is_muted(event.page, (uint8_t)pad)) { return; }

  sample_voice_live[pad] = live_input;
  sample_sounding_layer[pad] = event.layer;
  ...
  if (slot.loop_enabled && slot.loop_whole_sample) { play_sample_whole_loop(pad); }
    ...
    play_sample_once(pad, offset,
                     !live_input && slot.beatAnchorValid());
  }
}
...
static void clear_post_chop_prompt(bool redraw)
{
  post_chop_prompt_active = false;
  post_chop_count = 0;
  post_chop_slot_count = 0;
  post_chop_nominal_length_msec = 0;
  post_chop_fit_to_beat = false;
  clear_status_message(false);
  if (redraw) {
    request_wave_draw();
    request_all_fn_draw();
...
static bool post_chop_event_has_earlier_start(const std::vector<loop_event_t>& events,
                                               const loop_event_t& event,
                                               uint32_t boundary)
{
  return std::any_of(events.begin(), events.end(), [&event, boundary](const loop_event_t& candidate) {
    return candidate.page == event.page && candidate.pad == event.pad
        && candidate.layer == event.layer && candidate.type == loop_event_type_t::note_on
        && candidate.pos_ms < boundary;
  });
}

static bool apply_post_chop_timing(bool make_loop)
{
  if (!post_chop_prompt_active || post_chop_count == 0
   || post_chop_slot_count == 0
   || post_chop_nominal_length_msec < loop_min_length_ms) { return false; }

  uint32_t target_length = post_chop_nominal_length_msec;
  uint32_t working_length = loop_length_fixed && loop_length_msec >= loop_min_length_ms
    ? loop_length_msec : target_length;
  const bool had_fixed_loop = loop_length_fixed && loop_length_msec >= loop_min_length_ms;
  uint8_t fitted_audio_repeats = 0;
  // An Audio Beat is an actual PCM clock and cannot be stretched by moving
  // sequence timestamps. DONE therefore leaves it unchanged. MAKE LOOP with
  // FIT may select the same tempo octave at 1/2/4 PCM repeats.
  if (background_loop.isValid() && beat_format == beat_format_t::audio && had_fixed_loop) {
    if (!make_loop) {
      target_length = loop_length_msec;
    } else if (post_chop_fit_to_beat && background_loop.sample_rate) {
      const uint32_t cycle_length = std::max<uint32_t>(loop_min_length_ms,
        (uint32_t)(((uint64_t)background_loop.frames * 1000u
                  + background_loop.sample_rate / 2u) / background_loop.sample_rate));
      static constexpr uint8_t repeat_choices[] = { 1, 2, 4 };
      const uint32_t desired_length = target_length;
      uint32_t best_error = UINT32_MAX;
      for (uint8_t repeats : repeat_choices) {
        const uint32_t candidate = cycle_length * repeats;
        const uint32_t error = candidate > desired_length
          ? candidate - desired_length : desired_length - candidate;
        if (error < best_error) {
          fitted_audio_repeats = repeats;
          target_length = candidate;
          best_error = error;
        }
      }
    }
  ...
  {
    loop_events_guard_t guard;
    events.reserve(loop_events.size() + post_chop_slot_count);
    for (const auto& event : loop_events) {
      if (!make_loop || event.page != performance_page_t::sample) {
        events.push_back(event);
      }
    }
  }

  if (!had_fixed_loop && !make_loop) { return true; }

  if (had_fixed_loop && make_loop) {
    // Keep the new 4/8-slot phrase as the destination. Bring the old Rec into
    // the same tempo octave before the small final stretch: long Rec keeps its
    // first half, short Rec is duplicated in full (including Pattern Beat).
    while (length_at_least_150_percent(working_length, target_length)) {
      const uint32_t half = std::max<uint32_t>(loop_min_length_ms, working_length / 2u);
      std::vector<loop_event_t> folded;
      folded.reserve(events.size());
      for (auto event : events) {
        if (event.pos_ms < half) {
          folded.push_back(event);
        } else if (event.type == loop_event_type_t::note_off
                && post_chop_event_has_earlier_start(events, event, half)) {
          event.pos_ms = half - 1u;
          folded.push_back(event);
        } else if (event.type == loop_event_type_t::pitch_bend_center) {
          const bool earlier_bend = std::any_of(events.begin(), events.end(),
            [&event, half](const loop_event_t& candidate) {
              return candidate.page == event.page && loop_event_is_pitch_bend(candidate.type)
                  && candidate.type != loop_event_type_t::pitch_bend_center
                  && candidate.pos_ms < half;
            });
          if (earlier_bend) {
            event.pos_ms = half - 1u;
            folded.push_back(event);
      ...
      events.swap(folded);
      working_length = half;
      if (working_length == loop_min_length_ms) { break; }
    }
    while (length_below_75_percent(working_length, target_length)) {
      if (events.size() * 2u + post_chop_slot_count > loop_event_max) { return false; }
      const size_t original_count = events.size();
      events.reserve(original_count * 2u + post_chop_slot_count);
      for (size_t i = 0; i < original_count; ++i) {
        loop_event_t copy = events[i];
        copy.pos_ms += working_length;
        events.push_back(copy);
      }
      working_length *= 2u;
    }
  } else if (had_fixed_loop) {
    // DONE keeps the old data intact. Move the proposed 4/8-slot length by
    // powers of two until only a modest tempo correction remains.
    target_length = octave_match_length(
      target_length, working_length, loop_min_length_ms, UINT32_MAX);
  }

  if (working_length == 0 || target_length < loop_min_length_ms) { return false; }
  for (auto& event : events) {
    event.pos_ms = (uint32_t)(((uint64_t)event.pos_ms * target_length
                            + working_length / 2u) / working_length);
    if (event.pos_ms >= target_length) { event.pos_ms = target_length - 1u; }
  }

  uint16_t new_layer = 0;
  if (make_loop) {
    const uint8_t arranged = std::min<uint8_t>(post_chop_count, post_chop_slot_count);
    if (events.size() + arranged > loop_event_max) { return false; }
    new_layer = loop_layer_seq++;
    if (new_layer == 0) { new_layer = loop_layer_seq++; }
    for (uint8_t order = 0; order < arranged; ++order) {
      const uint8_t pad = display_order_to_pad(order);
      const uint32_t pos = (uint32_t)(((uint64_t)order * target_length)
                                   / post_chop_slot_count);
      events.emplace_back(performance_page_t::sample, pad,
                          loop_event_type_t::note_on, pos, new_layer);
    }
  }

  const bool was_playing = loop_playing;
  if (was_playing) { loop_toggle_play(); }
  stop_all_audio(false);
  if (make_loop && !post_chop_fit_to_beat && background_loop.isValid()
   && beat_format != beat_format_t::pattern) {
    clear_background_loop();
  } else if (fitted_audio_repeats != 0 && background_loop.isValid()) {
    background_loop.loop_repeats = fitted_audio_repeats;
  }
  {
    loop_events_guard_t guard;
  ...
  if (make_loop && new_layer != 0) {
    loop_undo_history[(uint8_t)performance_page_t::sample].push_back(new_layer);
  }
  loop_length_fixed = true;
  loop_length_msec = target_length;
  loop_prev_pos_ms = 0;
  ...
  loop_record_enabled = true;
  if (beat_format == beat_format_t::pattern) {
    // The transformed events now form one explicit complete Pattern cycle.
    background_loop.loop_repeats = 1;
    beat_pattern_base_length_msec = target_length;
    beat_tempo_bpm_x2 = infer_pattern_bpm_x2(target_length);
    beat_tempo_reference_bpm_x2 = beat_tempo_bpm_x2;
    beat_tempo_reference_base_length_msec = target_length;
  }
  ...
  sampler_audio_t::setFxQuantizeStepMs(loop_quantize_step_ms(loop_length_msec));
  invalidate_loop_timeline_cache();
  if (was_playing && !make_loop) { loop_toggle_play(); }
  return true;
}

static void dismiss_post_chop_prompt(bool redraw)
{
  if (!post_chop_prompt_active) { return; }
  const bool timing_ok = apply_post_chop_timing(false);
  clear_post_chop_prompt(redraw);
  if (!timing_ok) { show_status_message("TIMING UNCHANGED", 1400, false); }
  save_resume_kit();
}

static bool apply_post_chop_make_loop(void)
{
  if (!apply_post_chop_timing(true)) { return false; }
  clear_post_chop_prompt(false);
  if (current_page != performance_page_t::sample) {
    set_performance_page(performance_page_t::sample);
  }
  if (current_mode != sampler_mode_t::mode_loop) { set_mode(sampler_mode_t::mode_loop); }
  save_resume_kit();
  loop_toggle_play();
  request_grid_draw();
  request_wave_draw();
...
static void loop_record_pad(int pad)
{
  ...
  if (anchor_sync) {
    ...
    trigger_loop_event({ (uint8_t)pad, loop_event_type_t::note_on, raw_pos, layer }, true);
...
static void remap_synth_pad_references(uint8_t from, uint8_t to)
{
  if (from == to || from >= def::pad::pad_count || to >= def::pad::pad_count) { return; }
  bool changed = false;
  for (const performance_page_t page : { performance_page_t::melody,
                                         performance_page_t::bass,
                                         performance_page_t::chord }) {
    auto& settings = page_settings(page);
    if (settings.source != synth_tone_source_t::pad || settings.pad != from) { continue; }
    stop_synth_page(page);
    settings.pad = to;
    changed = true;
  }
  if (external_midi_sound == external_midi_sound_t::pad && external_midi_pad == from) {
    external_midi_pad = to;
    changed = true;
  }
  if (!changed) { return; }
  repair_pitched_pad_sources();
...
static bool sample_move_to_empty(uint8_t from, uint8_t to)
{
  ...
  remap_synth_pad_references(from, to);
...
static bool copy_moved_sample_back(void)
{
  ...
  // The first target tap temporarily performed a Move and remapped every
  // synth reference. Converting that gesture into Copy restores the original
  // source Pad, so restore those references as well.
  remap_synth_pad_references((uint8_t)target, (uint8_t)source);
  ...
  sample_move_copy_target_pad = -1;
  char copied[24];
  snprintf(copied, sizeof(copied), "COPIED TO P%u",
           (unsigned)pad_display_number((uint8_t)target));
  show_status_message(copied, 1000, false);
  request_header_draw();
  request_wave_draw();
...
static bool execute_sample_move_or_mix(int target)
{
  ...
  if (ok) {
    ...
    } else if (!target_was_empty) {
      char mixed[24];
      snprintf(mixed, sizeof(mixed), "MIXED INTO P%u",
               (unsigned)pad_display_number((uint8_t)target));
      show_status_message(mixed, 1200, false);
    }
    request_header_draw();
    request_wave_draw();
...
{
  const bool sound_surface = current_mode == sampler_mode_t::mode_rec
                          && current_page != performance_page_t::drum;
  const bool sampler_play_surface = current_page == performance_page_t::sample
                                 && current_mode == sampler_mode_t::mode_play;
  return !menu_visible && !loop_playing && edit_pad < 0 && recording_pad < 0
      && (sound_surface || sampler_play_surface);
}

static bool sample_add_armed_active(void)
{
...
static bool toggle_current_pad_mute(int pad)
{
  ...
  muted = !muted;
  if (muted) {
    // Pad Mute is also a sequence gate. Stop a recorded Sample loop that is
    // already sounding, but never cut a Pad currently owned by the player.
    if (current_page == performance_page_t::sample && !sample_voice_live[pad]) {
      stop_sample_grid_loop(pad);
    }
  }
  request_pad_draw(pad);
...
static bool shared_sample_page_pad_press(int pad)
{
  ...
  if (current_page == performance_page_t::drum) {
    ...
    if (beat_format == beat_format_t::pattern && beat_pool_t::slot[pad].isValid()) {
      stop_beat_sample_page_audition();
      // SOUND is an editing/selection surface. While the loop runs, Pad taps
      // only move focus; explicit Fn1 preview uses an independent voice.
      if (!loop_playing) { trigger_beat_pad((uint8_t)pad, true); }
    } else {
      show_status_message(beat_format == beat_format_t::audio
        ? "AUDIO BEAT" : "SELECT BEAT IN MENU", 1200, false);
    }
    request_wave_draw();
    request_pad_state_draw(pad);
    return true;
  ...
  auto& source = sampler_pool_t::slot[pad];
  if (!source.isValid()) {
    clear_menu_preview();
    shared_sample_assign_pad = -1;
    show_status_message("EMPTY SAMPLE", 1000, false);
    request_wave_draw();
    return true;
  }
  ...
  const bool confirm = sample_edit_armed_pad == pad
                    && shared_sample_assign_pad == pad
                    && shared_sample_assign_page == current_page;
  if (!confirm) {
    // This surface is a browser first: one Pad can be auditioned at a time,
    // and no synth setting changes until the same Pad is tapped again.
    clear_menu_preview();
    ...
    const uint32_t start = source.reverse && source.playEnd() > 0
      ? source.playEnd() - 1 : source.playStart();
    if (!loop_playing
  ...
  // Do not assign on the press edge. A short release confirms SET/EDIT while
  // holding the same press turns it into the shared Edit-or-Move gesture.
  clear_menu_preview();
  const auto& settings = page_settings(current_page);
  sound_pad_release_action = settings.source == synth_tone_source_t::pad
                          && settings.pad == (uint8_t)pad
    ? sound_pad_release_action_t::edit : sound_pad_release_action_t::assign;
  sample_edit_armed_pad = -1;
  sample_edit_pending_pad = pad;
  begin_hold_progress(hold_progress_kind_t::sample_move, pads[pad].press_msec,
                      sample_move_hold_ms, performance_page_colors[(uint8_t)current_page],
                      sound_pad_release_action == sound_pad_release_action_t::assign
                        ? "HOLD: EDIT / MOVE" : "HOLD: MOVE",
                      "RELEASE EDIT / TAP MOVE");
  request_wave_draw();
  request_pad_state_draw(pad);
  return true;
}
...
static bool play_sound_page_preview(void)
{
  ...
  if (current_page == performance_page_t::drum
   && beat_format == beat_format_t::audio) {
    if (!background_loop.isValid()) { return false; }
    pcm = background_loop.pcm;
    frames = background_loop.frames;
    sample_rate = background_loop.sample_rate;
    volume_q8 = background_loop.volume_q8;
  } else {
    if (pad < 0 || pad >= (int)def::pad::pad_count) { return false; }
  ...
  const uint32_t duration_ms = (uint32_t)std::max<uint64_t>(
    1, ((uint64_t)frames * 1000u + sample_rate - 1u) / sample_rate);
  synth_menu_preview_stop_msec = M5.millis() + duration_ms;
  request_fn_draw(0);
  return true;
}

static bool toggle_sound_page_preview(void)
{
  if (sound_page_preview_active && synth_menu_preview_sample_active) {
    clear_menu_preview();
    request_fn_draw(0);
    return true;
  }
  if (current_page == performance_page_t::drum
   && beat_format == beat_format_t::audio && loop_playing) {
    show_status_message("BEAT IS PLAYING", 1000, false);
    request_fn_draw(0);
    return false;
  }
  if (play_sound_page_preview()) { return true; }
  show_status_message("SELECT A SOUND", 1000, false);
  request_fn_draw(0);
  return false;
}

static bool shared_sample_page_pad_release(int pad)
{
  if (current_mode != sampler_mode_t::mode_rec
   || current_page == performance_page_t::sample) { return false; }
  if (current_page == performance_page_t::drum) {
    request_pad_state_draw(pad);
    return true;
  }
  ...
  if (sample_edit_pending_pad == pad) {
    const sound_pad_release_action_t action = sound_pad_release_action;
    sample_edit_pending_pad = -1;
    sound_pad_release_action = sound_pad_release_action_t::none;
    cancel_hold_progress(hold_progress_kind_t::sample_move);
    if (action == sound_pad_release_action_t::edit) {
      enter_edit(pad);
      request_grid_draw();
      return true;
    }
    if (action == sound_pad_release_action_t::assign) {
      auto& settings = page_settings(current_page);
      stop_synth_page(current_page);
      settings.source = synth_tone_source_t::pad;
      settings.pad = (uint8_t)pad;
      sample_edit_armed_pad = pad;
      shared_sample_assign_pad = (int8_t)pad;
      ...
      apply_synth_tones(true);
      char message[32];
      snprintf(message, sizeof(message), "%s ASSIGNED: P%u",
               performance_page_names[(uint8_t)current_page],
               (unsigned)pad_display_number((uint8_t)pad));
      show_status_message(message, 1200, false);
      save_resume_kit();
      request_wave_draw();
      request_grid_draw();
      request_pad_state_draw(pad);
  ...
  if (current_page != performance_page_t::drum && sample_browser_preview_trigger >= 0) {
    release_synth_trigger(current_page, (uint8_t)sample_browser_preview_trigger);
    sample_browser_preview_trigger = -1;
  }
  request_pad_state_draw(pad);
...
static void stop_shared_sample_page_preview(performance_page_t page)
{
  if (page == performance_page_t::sample) { stop_sample_page_audition(); }
  if (page == performance_page_t::drum) { stop_beat_sample_page_audition(); }
  if (sample_browser_preview_trigger >= 0
   && page != performance_page_t::sample && page != performance_page_t::drum) {
    release_synth_trigger(page, (uint8_t)sample_browser_preview_trigger);
  }
  sample_browser_preview_trigger = -1;
  clear_menu_preview();
  shared_sample_assign_pad = -1;
...
static int first_valid_sample_surface_pad(performance_page_t page)
{
  if (page == performance_page_t::drum) {
    if (beat_format != beat_format_t::pattern) { return -1; }
    for (uint8_t pad = 0; pad < def::pad::pad_count; ++pad) {
      if (beat_pool_t::slot[pad].isValid()) { return pad; }
    }
    return -1;
  }
...
static void prepare_shared_sample_page(performance_page_t page)
{
...
static void pad_press(int pad) {
  ...
  mark_sound_priority();
  // The completion notice is modal only for Fn2. A Pad tap dismisses it and
  // continues as the normal SOUND audition, which keeps the next gesture
  // immediate and avoids turning all twelve Pads into duplicate OK buttons.
  if (post_chop_prompt_active) { dismiss_post_chop_prompt(); }
  if (edit_pad >= 0) {
    const uint8_t number = pad_display_number((uint8_t)pad);
  ...
  if (current_mode == sampler_mode_t::mode_loop && fn_pressed[2]) {
    ...
    if (!edit_confirmed) {
      ...
      if (!loop_playing) { play_sample_page_audition(pad); }
...
static void pad_release(int pad) {
  ...
  if (sample_move_source_pad == pad) {
    // Holding without choosing a destination is the universal Edit gesture.
    cancel_sample_move();
    enter_edit(pad);
    request_grid_draw();
    return;
  ...
  if (sample_move_copy_source_pad == pad) {
    const int target = sample_move_copy_target_pad;
    cancel_sample_move();
    if (target >= 0 && target < (int)def::pad::pad_count) {
      char moved[24];
      snprintf(moved, sizeof(moved), "MOVED TO P%u",
               (unsigned)pad_display_number((uint8_t)target));
      show_status_message(moved, 1000, false);
    }
    request_pad_state_draw(pad);
    return;
  }
  if (!slot.isValid() && sample_add_available()) {
    ...
    // deliberate gesture chooses Import or Record. Preserve the armed state,
    // but remove the direct-LCD prompt immediately on finger release.
    if (sample_add_armed_pad == pad) {
      if (sample_add_status_active) { clear_status_message(false); }
      request_pad_state_draw(pad);
      return;
  ...
  if (shared_sample_page_pad_release(pad)) { return; }
  if (pad_repeat_next_msec[pad] || pad_repeat_transport_locked[pad]) {
...
{
  switch (page) {
  case performance_page_t::drum:   return mixer_part_t::beat;
...
static uint16_t mixer_scaled_volume_q8(mixer_part_t part, uint16_t base_q8)
{
  ...
  if (index >= mixer_part_count) { return 0; }
  // BGM has no separate live instrument. Every other Mute is a sequence gate,
...
{
  switch (part) {
  case mixer_part_t::beat:
    {
    const uint16_t background_q8 = (uint16_t)std::min<uint32_t>(512,
      ((uint32_t)background_loop.volume_q8 * beat_volume + 50) / 100);
    sampler_audio_t::setVoiceVolumeQ8(background_loop_voice,
      mixer_scaled_volume_q8(part, background_q8));
    for (uint8_t voice = 0; voice < beat_voice_count; ++voice) {
      const int8_t pad = beat_voice_pad[voice];
      if (pad < 0 || pad >= (int8_t)def::pad::pad_count) { continue; }
      const auto& slot = beat_pool_t::slot[(uint8_t)pad];
      uint16_t base_q8 = (uint16_t)std::min<uint32_t>(512,
        ((uint32_t)slot.volume_q8 * beat_volume + 50) / 100);
      sampler_audio_t::setVoiceVolumeQ8(beat_voice_base + voice,
...
static void set_mixer_part_muted(mixer_part_t part, bool muted)
{
  ...
  if (muted && (part == mixer_part_t::bass
    ...
    const performance_page_t page = part == mixer_part_t::bass
      ...
      : part == mixer_part_t::melody ? performance_page_t::melody
                                     : performance_page_t::chord;
    for (uint8_t pad = 0; pad < def::pad::pad_count; ++pad) {
      if (!synth_trigger_state[(uint8_t)page][pad].live) {
        release_synth_trigger(page, pad);
      }
    }
    if (page == performance_page_t::melody || page == performance_page_t::bass) {
  ...
  } else if (muted && part == mixer_part_t::sampler) {
    for (uint8_t pad = 0; pad < def::pad::pad_count; ++pad) {
      if (!sample_voice_live[pad]) { stop_sample_grid_loop(pad); }
    }
  } else if (muted && part == mixer_part_t::beat) {
    if (beat_format == beat_format_t::audio) {
      sampler_audio_t::stop(background_loop_voice);
    } else {
      stop_beat_voices(false);
    }
  } else if (!muted && part == mixer_part_t::beat
          && beat_format == beat_format_t::audio && loop_playing) {
    play_background_loop_at(loop_pos_ms(M5.millis()));
...
static void set_performance_page(performance_page_t page)
{
  ...
  stop_shared_sample_page_preview(previous_page);
  ...
  current_page = page;
  prepare_shared_sample_page(current_page);
...
static void set_mode(sampler_mode_t mode) {
  ...
  if (previous_mode == sampler_mode_t::mode_rec) {
    stop_shared_sample_page_preview(current_page);
  }
  ...
  if (mode == sampler_mode_t::mode_fx || leaving_fx
   || mode == sampler_mode_t::mode_rec
   || previous_mode == sampler_mode_t::mode_rec) {
...
static void handle_fn_button(int fn, bool press)
{
  ...
  // Fn2 is both the CHOP command and the post-CHOP decision control. Keep
  // its release captured even when a completed long press changes mode and
  // clears the prompt before the physical button comes back up.
  if (fn == 1 && post_chop_fn2_engaged) {
    if (!press) {
      const uint32_t held = performance_event_time() - fn_press_msec[fn];
      cancel_hold_progress(hold_progress_kind_t::post_chop_make_loop);
      post_chop_fn2_engaged = false;
      if (post_chop_prompt_active) {
        if (held >= post_chop_make_loop_hold_msec) {
          if (!apply_post_chop_make_loop()) {
            dismiss_post_chop_prompt(false);
            show_status_message("MAKE LOOP FAILED", 1600, false);
            request_wave_draw();
            request_all_fn_draw();
          }
        } else {
          dismiss_post_chop_prompt();
        }
      }
    }
    request_fn_draw(fn);
  ...
  if (fn == 2 && post_chop_fn3_engaged) {
    if (!press) { post_chop_fn3_engaged = false; }
    request_fn_draw(fn);
    return;
  }
  if (post_chop_prompt_active) {
    if (fn == 1) {
      if (press) {
        post_chop_fn2_engaged = true;
        begin_hold_progress(hold_progress_kind_t::post_chop_make_loop,
                            fn_press_msec[fn], post_chop_make_loop_hold_msec,
                            0xF0C050u, post_chop_prompt_action_text(),
                            "RELEASE TO APPLY");
      }
      request_fn_draw(fn);
      return;
    ...
    if (fn == 2) {
      if (press) {
        post_chop_fn3_engaged = true;
        dismiss_post_chop_prompt();
      }
      request_fn_draw(fn);
      return;
    }
    // Preview remains useful after CHOP. It is a normal action, so dismiss
    // the decision first and let the existing SOUND Fn1 path continue.
    if (press) { dismiss_post_chop_prompt(); }
  }
  if (edit_pad >= 0) {
    if (!press) {
  ...
  if (current_mode == sampler_mode_t::mode_rec && fn == 0) {
    // SOUND Fn1 is an isolated audition transport. Loop playback keeps
    // running underneath and remains available through Enc1 all-stop.
    if (press) { toggle_sound_page_preview(); }
    request_fn_draw(fn);
    return;
  }
  if (common_fn_mode()) {
...
static void process_bitmask(uint32_t bitmask, uint32_t event_msec) {
  ...
  if (wifi_update_active || startup_update_check_active || startup_update_check_returning) {
    ...
    // refuses the request, and every performance/menu input remains blocked.
    if (pressed_edge) {
      menu_consumed_release_mask |= pressed_edge;
      if (wifi_update_active) { cancel_wifi_update(); }
      else if (startup_update_check_active) { cancel_startup_update_check(); }
    }
    return;
  }
  if (wifi_file_server_qr_active) {
...
static const sample_source_t* find_builtin_sample_source(const char* name)
{
  if (!name) { return nullptr; }
  for (const auto& source : builtin_samples) {
    if (!strcmp(source.name, name)) { return &source; }
  }
  return nullptr;
}
...
static bool load_builtin_beat_sounds(void)
{
  ...
  for (uint8_t order = 0; order < def::pad::pad_count; ++order) {
    draw_busy_status_dots_tick();
    const uint8_t pad = display_order_to_pad(order);
    const auto* source = find_builtin_sample_source(sounds[order].source);
    if (!source || !beat_pool_t::loadWav(pad, beat_pad_labels[order],
                                         source->data, source->size())) {
      ok = false;
      continue;
    }
    auto& slot = beat_pool_t::slot[pad];
    slot.pitch_q8 = sounds[order].pitch_q8;
    snprintf(slot.file_path, sizeof(slot.file_path), "builtin:%s", source->name);
  }
  // P8/P12 are the closed/open hat pair. `false` means membership in the
  // Beat choke group, not that the Pad itself is disabled.
...
static bool clear_beat_pattern(void)
{
  // Clear the performed Pattern, not its Pad kit. If the current Beat is
  // Audio (or empty), switch to the built-in Pattern kit without discarding
  // the other parts' recorded events.
  if (beat_format != beat_format_t::pattern) {
    clear_background_loop();
    if (!load_builtin_beat_sounds()) {
      beat_pool_t::clear();
...
static void set_beat_repeat(uint8_t repeats)
{
  repeats = repeats <= 1 ? 1 : repeats <= 2 ? 2 : 4;
  ...
  uint32_t base_length = beat_pattern_base_length_msec;
  if (!base_length) {
    base_length = std::max<uint32_t>(loop_min_length_ms,
      loop_length_msec / std::max<uint8_t>(1, previous_repeats));
  }
  if (!base_length) { return; }

  // A Pattern repeat is event duplication, not time-stretching. Treat the
  // first cycle as the editable source and rebuild only Beat-page events;
  // every other part remains at its recorded absolute position.
  std::vector<loop_event_t> pattern_cycle;
  {
    loop_events_guard_t guard;
    pattern_cycle.reserve(loop_events.size());
    for (const auto& event : loop_events) {
      if (event.page == performance_page_t::drum && event.pos_ms < base_length) {
        pattern_cycle.push_back(event);
      }
    }
    loop_events.erase(std::remove_if(loop_events.begin(), loop_events.end(),
      [](const loop_event_t& event) { return event.page == performance_page_t::drum; }),
      loop_events.end());
  ...
  beat_pattern_base_length_msec = base_length;
  const uint32_t now = M5.millis();
  const uint32_t new_length = base_length * repeats;
  uint32_t position = loop_playing ? loop_pos_ms(now) : 0;
  if (new_length) { position %= new_length; }
  loop_length_msec = new_length;
  loop_start_msec = now - position;
...
{
  for (uint8_t order = 0; order < def::pad::pad_count; ++order) {
    if (beat_midi_note_map[order] == note) { return order; }
  }
  if (note <= 36) { return 0; }
  if (note == 38 || note == 40) { return 1; }
  if (note == 37) { return 2; }
  if (note == 39) { return 3; }
  if (note <= 42) { return 4; }
  if (note <= 44) { return 5; }
  if (note <= 47) { return note == 46 ? 11 : 6; }
  if (note == 49 || note == 52 || note == 55 || note == 57) { return 9; }
  if (note == 51 || note == 53 || note == 59) { return 10; }
  if (note == 54 || note == 56 || note == 58) { return 8; }
...
static bool parse_midi_beat_file(const char* path, parsed_beat_pattern_t* pattern)
{
  ...
  const int loaded = kp::storage_sd.loadFromFileToMemory(path, data, (size_t)file_size);
  if (loaded != file_size || memcmp(data, "MThd", 4) != 0) {
    free(data);
    return false;
  }
  ...
  const uint32_t header_size = be32(data + 4);
  if (header_size < 6 || 8u + header_size > (uint32_t)file_size) { free(data); return false; }
  const uint16_t division = be16(data + 12);
  if (division == 0 || (division & 0x8000u)) { free(data); return false; }
  uint32_t tempo_us = 500000;
  bool tempo_found = false;
  uint32_t max_tick = 0;
  std::vector<midi_hit_t> hits;
  size_t cursor = 8u + header_size;
  while (cursor + 8 <= (size_t)file_size) {
    const uint32_t chunk_size = be32(data + cursor + 4);
    const size_t chunk_start = cursor + 8;
    const size_t chunk_end = chunk_start + chunk_size;
    if (chunk_end > (size_t)file_size) { break; }
    if (memcmp(data + cursor, "MTrk", 4) != 0) { cursor = chunk_end; continue; }
    size_t pos = chunk_start;
    uint32_t tick = 0;
    uint8_t running = 0;
    auto read_vlq = [&](uint32_t* value) -> bool {
      uint32_t result = 0;
      for (uint8_t n = 0; n < 4 && pos < chunk_end; ++n) {
        const uint8_t byte = data[pos++];
        result = (result << 7) | (byte & 0x7Fu);
        if (!(byte & 0x80u)) { *value = result; return true; }
      }
      return false;
    };
    while (pos < chunk_end) {
      uint32_t delta = 0;
      if (!read_vlq(&delta)) { break; }
      tick += delta;
      max_tick = std::max(max_tick, tick);
      if (pos >= chunk_end) { break; }
      uint8_t status = data[pos];
      if (status & 0x80u) { ++pos; if (status < 0xF0u) { running = status; } }
      ...
      if (status == 0xFFu) {
        ...
        uint32_t length = 0;
        if (!read_vlq(&length) || pos + length > chunk_end) { break; }
        if (type == 0x51u && length == 3 && !tempo_found) {
          tempo_us = ((uint32_t)data[pos] << 16) | ((uint32_t)data[pos + 1] << 8) | data[pos + 2];
          tempo_found = true;
        }
        pos += length;
        running = 0;
      ...
      const uint8_t bytes = (command == 0xC0u || command == 0xD0u) ? 1 : 2;
  ...
  const bool has_drum_channel = std::any_of(hits.begin(), hits.end(),
    [](const midi_hit_t& hit) { return hit.channel == 9; });
...
static void clear_active_beat(void)
{
  const beat_format_t previous = beat_format;
  stop_beat_voices();
  M5.delay(8);
  beat_pool_t::clear();
  clear_background_loop();
  if (previous == beat_format_t::pattern) {
    loop_clear_page_events(performance_page_t::drum);
    loop_reset_recording_state_if_empty();
  }
  beat_format = beat_format_t::none;
...
static bool load_builtin_background_loop(const char* builtin_id)
{
  ...
  draw_startup_loading_frame("LOADING BEAT");
...
static void save_loop_as_bgm(void)
{
  ...
  free(pcm);
  draw_all();
  show_status_message(ok ? "Audio Beat saved" : "Beat save failed", 2000, false);
}
...
static bool save_kit_to_storage(kp::storage_base_t& storage, const char* path)
{
  ...
  if (beat_format == beat_format_t::pattern) {
    for (uint8_t pad = 0; pad < def::pad::pad_count; ++pad) {
      ...
      if (!is_resume) {
        char asset_path[128];
        snprintf(asset_path, sizeof(asset_path), "%s/beatPad%02u.wav",
                 asset_dir.c_str(), (unsigned)pad_display_number(pad));
        if (!save_pcm_as_wav(storage, asset_path, slot.pcm, slot.frames, slot.sample_rate)) {
          return false;
        }
        file = asset_path;
      }
  ...
  if (!strcmp(beat_format_name, "pattern")) {
    ...
    for (JsonObject item : beat["pads"].as<JsonArray>()) {
        ...
        if (audio_size > 4 && audio_size <= 3200 * 1024) {
          ...
          if (audio_data) {
            const int audio_len = kp::storage_sd.loadFromFileToMemory(
              file, audio_data, (size_t)audio_size);
            loaded = audio_len > 4
                  && beat_pool_t::loadWav((uint8_t)pad, item["name"] | "BEAT",
                                          audio_data, (size_t)audio_len);
            free(audio_data);
          }
        }
      } else {
      ...
      if (loaded) {
        auto& slot = beat_pool_t::slot[pad];
        snprintf(slot.file_path, sizeof(slot.file_path), "%s", file);
        slot.volume_q8 = item["volume"] | 256;
        slot.pitch_q8 = item["pitch"] | 256;
        beat_pad_overlap[pad] = item["overlap"] | true;
      }
    }
  }
...
bool sampler_web_export_state(std::string& out)
{
  ...
  JsonArray builtin_patterns_json = doc["builtinBeatPatterns"].to<JsonArray>();
  for (const auto& pattern : builtin_beat_patterns) {
    JsonObject item = builtin_patterns_json.add<JsonObject>();
    item["name"] = pattern.token;
    item["file"] = std::string("pattern:") + pattern.token;
  }
...
static void service_sampler_web_command(void)
{
  ...
  if (strcmp(action, "setPad") == 0) {
    ...
    invalidate_sample_pad_grid_cache((uint8_t)pad);
    request_wave_draw();
    request_pad_draw((uint8_t)pad);
    return;
  }
  if (strcmp(action, "loadBeat") == 0 || strcmp(action, "loadBgm") == 0) {
    const char* path = doc["file"] | "";
  ...
  if (strcmp(action, "newBeatPattern") == 0) {
    start_new_pattern_beat();
    return;
  }
  if (strcmp(action, "clearBeat") == 0 || strcmp(action, "clearBgm") == 0) {
    clear_active_beat();
    return;
  }
  if (strcmp(action, "setLoop") == 0) {
    ...
    if (!doc["beatVolume"].isNull()) {
      const uint8_t requested = std::clamp<uint8_t>(doc["beatVolume"].as<uint8_t>(), 0, 100);
      beat_volume = (uint8_t)((requested / 20u) * 20u);
      apply_mixer_part(mixer_part_t::beat);
    }
    sampler_audio_t::setFxQuantizeStepMs(loop_quantize_step_ms(loop_length_msec));
    refresh_sample_grid_loop_intervals();
...
static void update(void)
{
  ...
  service_sample_asset_compaction(msec);
  ...
  if (!ui_surface_exclusive && !page_selector_visible
   ...
   && !performance_status_message_visible(msec)
  ...
  // Performance notices sit above wave/timeline/cursor layers. Their backing
  // image is the retained Wave Canvas, restored once when the notice expires.
  service_performance_status_overlay(msec);
}
