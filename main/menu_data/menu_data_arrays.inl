...
static constexpr menu_item_ptr menu_system[] = {
  ...
  // 1. Song: file lifecycle for the current song.
  MENU_BUILDER(mi_tree_t              , 1    , { "Song"             , "ソング"        }),
  MENU_BUILDER(mi_tree_t              ,  2   , {  "New Song"        , "新しいソング"   }),
  MENU_BUILDER(mi_new_song_t          ,   3  , {   "Blank"           , "空のソング"     }),
  MENU_BUILDER(mi_tree_t              ,   3  , {   "From Genre"      , "ジャンルから作成"}),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Simple"         , "シンプル"      }, def::app::data_type_t::data_song_preset_genre_simple,   0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Pop"            , "ポップス"      }, def::app::data_type_t::data_song_preset_genre_pop,      0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Rock"           , "ロック"        }, def::app::data_type_t::data_song_preset_genre_rock,     0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Dance"          , "ダンス"        }, def::app::data_type_t::data_song_preset_genre_dance,    0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Funk"           , "ファンク"      }, def::app::data_type_t::data_song_preset_genre_funk,     0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "R&B"            , "R&B"          }, def::app::data_type_t::data_song_preset_genre_rnb,      0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Jazz"           , "ジャズ"        }, def::app::data_type_t::data_song_preset_genre_jazz,     0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Latin"          , "ラテン"        }, def::app::data_type_t::data_song_preset_genre_latin,    0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Acoustic"       , "アコースティック"}, def::app::data_type_t::data_song_preset_genre_acoustic, 0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Ballad"         , "バラード"      }, def::app::data_type_t::data_song_preset_genre_ballad,   0, true ),
  MENU_BUILDER(mi_load_file_t         ,    4 , {    "Specialty"      , "特殊リズム"    }, def::app::data_type_t::data_song_preset_genre_specialty,0, true ),
  MENU_BUILDER(mi_tree_t              ,  2   , {  "Open Song"       , "ソングを開く"   }),
  MENU_BUILDER(mi_load_file_t         ,   3  , {   "Preset Song"    , "プリセットソング"}, def::app::data_type_t::data_song_preset_song, 0 ),
  MENU_BUILDER(mi_load_file_t         ,   3  , {   "User Song (SD)" , "ユーザソング(SD)"}, def::app::data_type_t::data_song_users ),
  MENU_BUILDER(mi_load_file_t         ,   3  , {   "Extra Song (SD)", "エクストラソング(SD)"}, def::app::data_type_t::data_song_extra ),
  MENU_BUILDER(mi_save_t              ,  2   , {  "Save Song"       , "ソングを保存"   }, def::app::data_type_t::data_song_users ),
  MENU_BUILDER(mi_web_filer_t         ,  2   , {  "Song Manager"    , "ソング管理"    }),
  MENU_BUILDER(mi_reset_song_t        ,  2   , {  "Reset Song"      , "ソングデータリセット"}),

  // 2. Arrange: define the whole-song feel and structure. Arranger joins this level when merged.
  MENU_BUILDER(mi_tree_t              , 1    , { "Arrange"          , "アレンジ"       }),
  MENU_BUILDER(mi_tree_t              ,  2   , {  "Genre"           , "ジャンル"       }),
  MENU_BUILDER(mi_load_file_t         ,   3  , {   "Simple"          , "シンプル"      }, def::app::data_type_t::data_song_preset_genre_simple,   0 ),
  ...
  MENU_BUILDER(mi_num_slot_t          ,  2   , {  "Number of Sections", "セクション数"}),

  // 3. Edit: musical content and section-level editing.
  MENU_BUILDER(mi_tree_t              , 1    , { "Edit"             , "編集"          }),
  MENU_BUILDER(mi_tree_t              ,  2   , {  "Chord Sequence"  , "コード進行"     }),
  MENU_BUILDER(mi_tree_t              ,   3  , {   "Load Sequence"  , "コード進行を開く"}),
  MENU_BUILDER(mi_load_progression_t  ,    4 , {    "Load Preset"   , "プリセットを開く"}, def::app::data_type_t::data_progression_preset ),
  MENU_BUILDER(mi_load_progression_t  ,    4 , {    "Load User's (SD)", "ユーザデータ(SD)"}, def::app::data_type_t::data_progression_users ),
  ...
  MENU_BUILDER(mi_reset_progression_t ,   3  , {   "Reset Sequence" , "コード進行をリセット"}),
  MENU_BUILDER(mi_melody_edit_t       ,  2   , {  "Melody"          , "メロディ"       }),
  MENU_BUILDER(mi_tree_t              ,  2   , {  "Section"         , "セクション"      }),
  ...
  MENU_BUILDER(mi_tree_t              ,  2   , {  "Part"            , "パート"          }),
  MENU_BUILDER(mi_part_settings_link_t,   3  , {   "Part 1"          , "パート1"         }, 0),
  MENU_BUILDER(mi_part_settings_link_t,   3  , {   "Part 2"          , "パート2"         }, 1),
  MENU_BUILDER(mi_part_settings_link_t,   3  , {   "Part 3"          , "パート3"         }, 2),
  MENU_BUILDER(mi_part_settings_link_t,   3  , {   "Part 4"          , "パート4"         }, 3),
  MENU_BUILDER(mi_part_settings_link_t,   3  , {   "Part 5"          , "パート5"         }, 4),
  MENU_BUILDER(mi_part_settings_link_t,   3  , {   "Part 6"          , "パート6"         }, 5),

  // 4. Play: performance behavior shared by normal and guide play.
  MENU_BUILDER(mi_tree_t              , 1    , { "Play"             , "演奏"          }),
  MENU_BUILDER(mi_play_mode_t         ,  2   , {  "Play Mode"       , "プレイモード"   }),
  MENU_BUILDER(mi_song_autorepeat_t   ,  2   , {  "Auto Repeat"     , "オートリピート" }),
  MENU_BUILDER(mi_recording_t         ,  2   , {  "Recording"       , "レコーディング" }),

  // 5-7 intentionally match Sampler for cross-product muscle memory.
  MENU_BUILDER(mi_tree_t              , 1    , { "External Device", "外部デバイス" }),
  MENU_BUILDER(mi_external_input_source_t, 2 , { "Input Source"    , "入力ソース"   }),
  MENU_BUILDER(mi_input_status_t      ,  2   , { "Input Status"    , "入力状態"     }),
  MENU_BUILDER(mi_input_assign_link_t ,  2   , { "Input Assign"    , "入力割り当て" }),
  MENU_BUILDER(mi_ble_connection_t    ,  2   , { "BLE MIDI Connection", "BLE MIDI接続" }),
  MENU_BUILDER(mi_ble_scan_t          ,   3  , { "Scan & Connect"  , "検索して接続" }),
  MENU_BUILDER(mi_ble_action_t        ,   3  , { "Forget Device"   , "接続先を解除" }, false),
  MENU_BUILDER(mi_ble_action_t        ,   3  , { "Reset BLE Connection", "BLE接続をリセット" }, true),
  MENU_BUILDER(mi_device_info_t       ,  2   , { "Device Info"     , "デバイス情報" }),
  MENU_BUILDER(mi_portc_midi_t        ,  2   , { "UART MIDI Output", "UART MIDI出力" }),
  MENU_BUILDER(mi_tree_t              ,  2   , { "InstaChord Link", "インスタコードリンク"}),
  MENU_BUILDER(mi_iclink_port_t       ,   3  , { "Connect"         , "接続方法"   }),
  MENU_BUILDER(mi_iclink_dev_t        ,   3  , { "Play Device"     , "演奏デバイス"}),
  MENU_BUILDER(mi_iclink_style_t      ,   3  , { "Play Style"      , "演奏スタイル"}),
  MENU_BUILDER(mi_tree_t              , 1    , { "Wi-Fi"           , "Wi-Fi"       }),
  MENU_BUILDER(mi_otaupdate_t         ,  2   , { "Update"          , "ファーム更新" }),
  MENU_BUILDER(mi_wifiap_t            ,  2   , { "Wi-Fi Setup"     , "Wi-Fi設定"    }),
  MENU_BUILDER(mi_web_filer_t         ,  2   , { "File Editor"     , "ファイル編集" }),
  MENU_BUILDER(mi_webserver_t         ,  2   , { "Web server"      , "Webサーバ"    }),
  ...
  MENU_BUILDER(mi_tree_t              ,    4 , { "Section Button", "セクションボタン" }),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 1" , "セクションBtn 1" },  1 - 1, def::mapping::target_t::device),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 2" , "セクションBtn 2" },  2 - 1, def::mapping::target_t::device),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 3" , "セクションBtn 3" },  3 - 1, def::mapping::target_t::device),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 4" , "セクションBtn 4" },  4 - 1, def::mapping::target_t::device),
  ...
  MENU_BUILDER(mi_tree_t              ,    4 , { "Section Button", "セクションボタン" }),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 1" , "セクションBtn 1" },  1 - 1, def::mapping::target_t::song),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 2" , "セクションBtn 2" },  2 - 1, def::mapping::target_t::song),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 3" , "セクションBtn 3" },  3 - 1, def::mapping::target_t::song),
  MENU_BUILDER(mi_ca_slotbutton_t     ,     5, { "Section Btn 4" , "セクションBtn 4" },  4 - 1, def::mapping::target_t::song),
  MENU_BUILDER(mi_tree_t              ,    4 , { "Ext Input"     , "拡張入力"     }),
...
static constexpr menu_item_ptr menu_part[] = {
  ...
  MENU_BUILDER(mi_program_root_t  , 1 , { "Tone"           , "音色"             }),
  MENU_BUILDER(mi_program_category_t, 2, { "Piano"         , "ピアノ"           }, 0),
  MENU_BUILDER(mi_program_category_t, 2, { "Chromatic"     , "鍵盤打楽器"       }, 1),
  MENU_BUILDER(mi_program_category_t, 2, { "Organ"         , "オルガン"         }, 2),
  MENU_BUILDER(mi_program_category_t, 2, { "Guitar"        , "ギター"           }, 3),
  MENU_BUILDER(mi_program_category_t, 2, { "Bass"          , "ベース"           }, 4),
  MENU_BUILDER(mi_program_category_t, 2, { "Strings"       , "ストリングス"     }, 5),
  MENU_BUILDER(mi_program_category_t, 2, { "Ensemble"      , "アンサンブル"     }, 6),
  MENU_BUILDER(mi_program_category_t, 2, { "Brass"         , "ブラス"           }, 7),
  MENU_BUILDER(mi_program_category_t, 2, { "Reed"          , "リード"           }, 8),
  MENU_BUILDER(mi_program_category_t, 2, { "Pipe"          , "パイプ"           }, 9),
  MENU_BUILDER(mi_program_category_t, 2, { "Synth Lead"    , "シンセリード"     }, 10),
  MENU_BUILDER(mi_program_category_t, 2, { "Synth Pad"     , "シンセパッド"     }, 11),
  MENU_BUILDER(mi_program_category_t, 2, { "Synth FX"      , "シンセFX"         }, 12),
  MENU_BUILDER(mi_program_category_t, 2, { "Ethnic"        , "民族楽器"         }, 13),
  MENU_BUILDER(mi_program_category_t, 2, { "Percussive"    , "打楽器"           }, 14),
  MENU_BUILDER(mi_program_category_t, 2, { "Sound FX"      , "効果音"           }, 15),
  MENU_BUILDER(mi_program_category_t, 2, { "Drum"          , "ドラム"           }, 16),
...
static constexpr menu_item_ptr menu_song_edit[] = {
  ...
  MENU_BUILDER(mi_open_main_menu_t    , 1 , { "More..."          , "詳細設定"        }),
  nullptr, // end of menu
};
...
static constexpr menu_item_ptr menu_autosong[] = {
  MENU_BUILDER(mi_song_play_menu_t    ,0     , { "Song Play Menu"   , "ソング演奏メニュー"   }),
  MENU_BUILDER(mi_auto_song_advance_t , 1    , { "Advance Mode"     , "進行方法"             }),
