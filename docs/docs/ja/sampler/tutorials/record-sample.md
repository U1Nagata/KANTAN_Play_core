...
内蔵マイクを使って、声、手拍子、身の回りの音をパッドへ録音します。
...
1. 録音したい音を出す準備をします
2. 先ほど選んだ空パッドを押し、そのまま長押しします
3. 画面が`SAMPLING / MIC INPUT`に変わったら音を出します
4. 録り終えたい位置でパッドを離します

<!-- IMAGE REC-01: SAMPLING / MIC INPUT画面。Pad番号も読める状態。 -->

録音できる長さは最大20秒です。ただし、Audio Beatとサンプラー音源は合計5MiBのメモリを共有するため、長いAudio Beatや多くのサンプルを読み込んだ状態では録音上限が短くなります。最初は「パン」「ヘイ」など、1秒前後の短い音から試すと結果を確認しやすくなります。

!!! tip
    パッドを押してから声を出すまで少し間が空いても構いません。録音後に無音部分を自動で取り除きます。

## 自動処理を待つ

パッドを離すと、次の処理が自動で行われます。
...
録音したパッドを短く1回押します。録音した音がPreviewされます。
...
| 表示 | 確認すること |
|---|---|
| `RECORDING TOO SHORT` | もう少し長くパッドを押して録音します |
| `NO AUDIO DETECTED` | マイクへ少し近づき、はっきりした音でもう一度試します |
| `NOT ENOUGH SAMPLE SPACE` | 不要なサンプルを削除するか、短い音で試します |
| `SAMPLE SAVE FAILED` | 空きメモリとSDカードを確認し、不要なサンプルを減らしてから再試行します |
