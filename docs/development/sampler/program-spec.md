...
ENC2/ENC3を回すと、ヘッダーとPadを残したまま上部情報エリア全体にパートセレクターを表示する。選択中のパートはページカラーの面と大きな太字、直前／次の候補は暗い小文字で表す。操作を止めるかエンコーダーを押すと確定する。表示中にPad、Fn、レバー、モードなどの演奏操作が入った場合は、選択中のパートを即時確定し、同じ入力イベントを演奏に使う。画面復元は発音より後に行う。
