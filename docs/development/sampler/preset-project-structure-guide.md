...
│   ├── activeSection          # 選択中Loop Section（0始まり）
│   ├── background             # Project共有Audio Beat。Sectionごとに複製しない
│   └── sections[]             # 1〜4個
│       ├── pattern            # Section固有Pattern Beat情報
│       └── events[]           # Section固有Rec。partはPerformance Part
├── synth
│   ├── key / scale / tuning
│   ├── bass
│   ├── melody
...
Project共有で、RecイベントとPattern BeatだけがLoop Section固有です。1 Sectionあたりのイベント上限は512、
Section数は最大4です。v10の`loop.events[]`は読込時にS1へ移行します。JSONを手作業で生成せず、本体または
対応ツールで保存して、`activeSection`と各`sections[]`の復元を確認してください。
